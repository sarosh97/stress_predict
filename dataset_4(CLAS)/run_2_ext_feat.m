...
for i = 1:length(subj)
    ...
    test_name = readtable(['D:\CLAS\Block_details\Part' num2str(subj(i)) '_Block_Details.csv']);
    ...
    for j = neutral_files    
        ...
        data_raw = readtable(['D:CLAS\Participants\Part' num2str(subj(i)) '\by_block\' test_name.EDA_PPGFile{j}]);
        data = data_raw.gsr;
    ...
    for j = stress_files    
        if strcmp(test_name.BlockType{j}, 'Math Test')
            data_raw = readtable(['D:\CLAS\Participants\Part' num2str(subj(i)) '\by_block\' test_name.EDA_PPGFile{j}(1:end-4) 'mathtest.csv']);
            data = data_raw.gsr;
        ...
        elseif strcmp(test_name.BlockType{j}, 'Stroop Test')
            data_raw = readtable(['D:\CLAS\Participants\Part' num2str(subj(i)) '\by_block\' test_name.EDA_PPGFile{j}(1:end-4) 'strooptest.csv']);
            data = data_raw.gsr;
            data_resampled = resample(data, new_fs, fs_e); %resampling for quicker processing
            data_e_stress = data_resampled(new_fs:end-new_fs); %chopping off the ends (about 1 fs_new) to remove transients
            t_e_stress = (0:length(data_e_stress)-1)'/new_fs;
        ...
        elseif strcmp(test_name.BlockType{j}, 'IQ Test')
            data_raw = readtable(['D:\CLAS\Participants\Part' num2str(subj(i)) '\by_block\' test_name.EDA_PPGFile{j}(1:end-4) 'IQtest.csv']);
            data = data_raw.gsr;
            data_resampled = resample(data, new_fs, fs_e); %resampling for quicker processing
            data_e_stress = data_resampled(new_fs:end-new_fs); %chopping off the ends (about 1 fs_new) to remove transients
            t_e_stress = (0:length(data_e_stress)-1)'/new_fs;
