...
filename = 'D:\Research Papers and Data\Wearable sensing\dataset_2';
...
for i = 1:length(subj)
    ...
    %time log
    tags = importdata([filename '\Raw_data\S' subj{i} '\tags_S' subj{i} '.csv']);
    tags = round(tags);

    %Indices for the duration of stress test
    n_e = [((tags(1)-tags(1))*fs_e+1:(tags(2)-tags(1))*fs_e-1)]';

    data_raw_e = importdata([filename '\Raw_data\S' subj{i} '\EDA.csv']);

    %first value of each file is timestamp (not same as first timestamp of 'logs')
    data_log_e = data_raw_e(1);

    %Selecting the data that starts from the timestamp given in 'tags' file
...
if win_shift_e == 0.25*fs_e
    save('features_stroop.mat', 'features')

elseif win_shift_e == 5*fs_e
    save('features_stroop_5.mat', 'features')

elseif win_shift_e == 30*fs_e
    save('features_stroop_30.mat', 'features')
end